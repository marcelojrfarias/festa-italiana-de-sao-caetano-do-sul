...
export function metrosParaLatLng(frame, [x, y]) {
...
export function latLngParaMetros(frame, [lat, lng]) {
  const origem = frame?.origem?.latlng;
  if (!origem) return null;
  const [lat0, lng0] = origem;
 ...
 */
export function cantosDaBarraca(b) {
...
export function barracaMaisProxima(barracas, ponto) {
  let melhor = null;
  let menor = Infinity;
